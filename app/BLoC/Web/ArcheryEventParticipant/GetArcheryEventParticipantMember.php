...
class GetArcheryEventParticipantMember extends Retrieval
{
    ...
    protected function process($parameters)
    {
        ...
        if (!is_null($status) && $status != 0) {
            $participant->where('archery_event_participants.status', $status);
            if ($status == 2) {
                $participant->orWhere(function ($query) use ($status) {
            ...
            if ($status == 4) {
                $participant->where("transaction_logs.expired_time", ">=", time());
            }
        }
        $participant->orderBy('archery_event_participants.created_at', 'DESC')->get();
        $participant_ids = [];
        $participants = [];
