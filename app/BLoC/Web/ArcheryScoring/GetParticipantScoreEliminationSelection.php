...
class GetParticipantScoreEliminationSelection extends Retrieval
{
    ...
    protected function process($parameters)
    {
        ...
        if ($category_detail->admin_id !== $admin->id) {
            $role = AdminRole::where("admin_id", $admin->id)->where("event_id", $category_detail->event_id)->first();
        ...
        if ($category_detail->type == "Individual") {
            $qualification_member = ArcheryScoring::getScoringRankByCategoryIdForEliminationSelection($event_category_id, 4, $session, true, $name, false);
