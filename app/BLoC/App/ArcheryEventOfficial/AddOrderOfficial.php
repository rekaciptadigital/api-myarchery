...
class AddOrderOfficial extends Retrieval
{
    ...
    protected function process($parameters)
    {
        ...
        // cek apakah user tersebut benar tergabung di club tersebut jika club id tidak 0
        if ($club_id != 0) {
            $club_member = ClubMember::where('club_id', $club_id)->where('user_id', $user_login->id)->first();
            if (!$club_member) {
                throw new BLoCException("Anda belum tergabung pada club tersebut");
            }
        }
        ...
        if ($is_exist->count() > 0) {
            foreach ($is_exist as $value) {
                if ($value->status == 1) {
                    throw new BLoCException("Anda telah terdaftar sebagai anggota official pada event ini");
                } elseif ($value->status_transaction_log == 4 && $value->expired_time > $time_now) {
                    throw new BLoCException("Transaksi anda sedang berlangsung, mohon selesaikan transaksi anda");
                }
            }
        }
        ...
        if ($official_count >= $archery_event_official_detail->quota) {
        ...
        if ($archery_event_official_detail->fee < 1) {
            $archery_event_official =  ArcheryEventOfficial::insertOrderOfficial($user_login->id, $club_id, $relation_id, $label, $archery_event_official_detail->id, 1);
            return [
        ...
        $archery_event_official =  ArcheryEventOfficial::insertOrderOfficial($user_login->id, $club_id, $relation_id, $label, $archery_event_official_detail->id);
